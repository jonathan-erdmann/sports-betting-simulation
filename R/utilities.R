...
get_estimated_return_percentile <- function(iBets) {
  ...
  sim_profit_loss <- get_simulation_profit_loss(sim_results, bets)
  estimated_percentile_values <- unname(quantile(sim_profit_loss, seq(0,1,0.001)))
  percentiles <- seq(0,1,0.001)
  profit_loss <- get_profit_loss(bets)
  
  estimated_return_percentile <- min(percentiles[estimated_percentile_values > get_profit_loss(bets)])
