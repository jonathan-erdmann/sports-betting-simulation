...
title_text <- paste0("Expected Cumulative Profit/Loss Distribution\n\tTotal Wagered: ", round(100*total_wagered,3), "%\n\tProbability of Loss: ", round(100*loss_probability,2), "%")

g <- ggplot(pnl_plot_data, aes(x = profit, y = cum_prob)) + geom_line() + geom_point(aes(color=win)) + scale_color_manual(values=c("red","blue"))
...
g <- ggplot(pnl_plot_data, aes(x = profit, y = prob)) + geom_point(aes(color=win)) + geom_line() + scale_color_manual(values=c("red","blue"))
g <- g + ggtitle(title_text)
g <- g + xlab("Profit") + ylab("Cumulative Probability") + theme_pander()
g <- g + xlim(c(-0.5, 0.5))
g
