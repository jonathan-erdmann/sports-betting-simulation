...
query <- paste("select * from bets where kelly_bet > 0 and date = ", today)
...
pnl_plot_data <- data.frame(table(round(sim_pnl,2))) %>% mutate(prob=Freq/sum(Freq), cum_prob=cumsum(prob)) %>% rename("profit"="Var1")
